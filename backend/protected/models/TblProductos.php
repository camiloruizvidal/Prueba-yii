...
class TblProductos extends CActiveRecord implements AjaxResponseInterface
{
	...
	public function rules()
	{
		...
		return array(
			array('description, reference, stock, currency, price', 'required'),
			array('stock', 'numerical', 'integerOnly'=>true),
			array('price', 'numerical'),
			array('description, reference, currency', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, description, reference, stock, currency, price', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return TblProductos the static model class
	 */
